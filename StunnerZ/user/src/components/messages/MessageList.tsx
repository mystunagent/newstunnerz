...
import { loadMoreMessages, deactiveConversation, loadMessages } from '@redux/message/actions';
...
class MessageList extends PureComponent<IProps> {
  ...
  purcharseMessage = () => {
    const { conversation, loadMessages: handleLoadMessages } = this.props;
    handleLoadMessages({ conversationId: conversation._id });
  }
      ...
      if (current._id) {
        tempMessages.push(
          <Message
            ...
            purcharseMessage={this.purcharseMessage}
          />
        );
      }
            ...
            <>
              <div aria-hidden className="mess-recipient">
                <span
                  className="recipient"
                  aria-hidden
                  onClick={() => conversation?.recipientInfo?.isPerformer && Router.push({ pathname: '/creator/profile', query: { username: conversation?.recipientInfo?.username || conversation?.recipientInfo?._id } }, `/${conversation?.recipientInfo?.username || conversation?.recipientInfo?._id}`)}
                >
              ...
              <div className="message-list-container" ref={this.messagesRef as any}>
                ...
                {this.renderMessages()}
                {!fetching && !conversation?.noreply && !message.items.length && <p className="text-center" style={{ margin: '30px 0' }}>Let&apos;s talk</p>}
                {/* {!conversation?.noreply && (!conversation.isSubscribed && (
                  <Link href={{ pathname: '/creator/profile', query: { username: conversation?.recipientInfo?.username || conversation?.recipientInfo?._id } }} as={`/${conversation?.recipientInfo?.username || conversation?.recipientInfo?._id}`}>
                    <div className="sub-text">Please follow to this creator to start the conversation!</div>
                  </Link>
                ))} */}
                {conversation.isBlocked && <div className="sub-text">This creator has blocked you!</div>}
              </div>
              {!conversation?.noreply && <Compose conversation={conversation} />}
            </>
          )
...
const mapDispatch = { loadMessages, loadMoreMessages, deactiveConversation };
