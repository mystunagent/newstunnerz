import { performerService } from '@services/performer.service';
import {
  Button,
  Col, Layout, message, Row,
  Spin
} from 'antd';
import { useEffect, useState } from 'react';
import './search-model-stream.less';
import { useDispatch } from 'react-redux';
import Router from 'next/router';
import { showSubscribePerformerModal } from '@redux/subscription/actions';
import BarTitleHome from '@components/common/base/bar-title-home';
import BannerComponent from '@components/common/banner';
import PerformerGridCardLiveStream from '@components/performer/grid-card-live';

type IProps = {
  user: any;
  settings: any;
  topBanners: any;
}

export default function SearchModelStream({ user, settings, topBanners }: IProps) {
  const limit = 6;
  const [modelStream, setModelStream] = useState<any>();
  const [loading, setLoading] = useState(false);
  const dispatch = useDispatch();
  const handleClick = (data) => {
    if (!user._id) {
      message.error('Please log in or register!', 5);
      Router.push('/');
      return;
    }
    if (user.isPerformer) return;
    if(data?.streamingStatus === 'private') {
      message.error(`${data?.name || data?.username} is currently in a private stream. Please try again later.`, 5);
      return;
    }
    if (data.liveInfo?.optionStream === 'subscribe' && !data.isSubscribed) {
      message.error('Please subscribe to join live chat!', 5);
      setTimeout(() => {
        Router.push(`/${data?.username}`);
      }, 3000)
      return;
    }
    Router.push(
      {
        pathname: '/streaming/details',
        query: { username: data?.username || data?._id }
      },
      `/streaming/${data?.username || data?._id}`
    );
  };

  const loadModelStream = async (page = 1) => {
    const offset = (page - 1) * limit;
    try {
      setLoading(true);
      let resp;
      if(user && user._id) {
        resp = await performerService.search({
          limit,
          offset,
          sortBy: 'online',
          type: 'live',
          sort: 'live'
        });
      }else {
        resp = await performerService.searchNoAuth({
          limit,
          offset,
          sortBy: 'online',
          type: 'live',
          sort: 'live'
        });
      }
      setModelStream(resp?.data);
      setLoading(false);
    } catch (error) {
      setLoading(false);
      const e = await error;
      message.error(e.message || 'An error occurred');
    }
  };

  useEffect(() => {
    loadModelStream();
  }, []);

  return (
    <Layout>
      {modelStream && modelStream.data.length > 0 && (
      <>
        <BarTitleHome title="Currently Live" />
        <div className="custom-model-live">
          <Row>
            {modelStream.data.filter((d) => d?._id !== user?._id).map((item) => (
              <Col key={item._id} md={8} xl={4} lg={4} xs={12}>
                <PerformerGridCardLiveStream redirect={() => handleClick(item)} performer={item} />
                {/* <div onClick={() => handleClick(item)} aria-hidden>
                  <div className="grid-card" style={{ backgroundImage: `url(${item?.avatar || '/static/no-avatar.png'})` }}>
                    <div className="live-status-card">
                      <div className="live-status-card-content">
                        <span>
                          {shortName(item.name) || shortName(item.username) || 'N/A'}
                        </span>
                        <div className="live-status-heart text-center">
                          <HeartTwoTone twoToneColor="#eb2f96" />
                          {' '}
                          Live
                        </div>
                      </div>
                    </div>
                  </div>
                </div> */}
              </Col>
            ))}
          </Row>
          <div className="text-center">
            <Button onClick={() => Router.push('/creator/all-live')}>Show All Live</Button>
          </div>
        </div>
      </>
      )}
      {loading && (
      <div className="text-center">
        <Spin />
      </div>
      )}
      {modelStream && modelStream.data.length > 0 && !loading && (
        <BannerComponent settings={settings} banners={topBanners} width={302.362} />
      )}
    </Layout>
  );
}
