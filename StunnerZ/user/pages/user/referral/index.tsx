...
function UserReferralPage() {
  ...
  const handlePageChange = async (key: 'earning' | 'users' | 'token_earning') => {
    if (key === 'earning') {
  ...
  const content = (
    <div>
      <p>
        {`Refer a creator - get ${settings?.u2pReferralCommission * 100 || 0}% on the creator revenue`}
      </p>
      <p>
        {`Refer a fan - get ${settings?.u2uReferralCommission * 100 || 0}% on the fan spends for 1 year`}
      </p>
    </div>
  );
