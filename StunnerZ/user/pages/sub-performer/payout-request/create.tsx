...
class SubPerformerPayoutRequestCreatePage extends React.PureComponent<Props, States> {
  ...
  calculateStatsPayout = async () => {
    ...
    const resp = await payoutRequestService.subPerformerCalculate({ sourceId: user?.infoSubPerformer?._id });
    resp?.data && this.setState({ statsPayout: resp.data });
  };

  async submit(data) {
    const { user } = this.props;
    if (data.requestTokens > user?.infoSubPerformer?.balance) {
    ...
    try {
      ...
      const body = { ...data, source: 'sub_performer', sourceId: user?.infoSubPerformer?._id };
      await payoutRequestService.subCreate(body);
    ...
    return (
      <Layout>
        ...
        <div className="main-container">
          <PageHeading title="Request Token Payout" icon={<NotificationOutlined />} />
          <PayoutRequestForm
            payout={{
              requestNote: '',
              requestTokens: 1,
...
export default connect(mapStateToProps)(SubPerformerPayoutRequestCreatePage);
