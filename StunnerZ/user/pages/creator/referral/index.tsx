...
import { Divider, Layout, message, Popover, Tabs } from "antd";
...
function ModelReferralPage({ user }: IProps) {
  ...
  const getReferralCode = async () => {
    try {
      ...
      setLinkReferral(`${window.location.origin}/auth/login?rel=${resp.data}`);
    } catch (e) {
      const err = await e;
  ...
  useEffect(() => {
    ...
    if (
      ...
      !user?.infoSubPerformer?.privilege.includes("referral")
    ) {
      Router.push("/");
      return;
    }
  ...
  const content = (
    <div>
      <p>
        {`Refer a creator - get ${
          settings?.p2pReferralCommission * 100 || 0
        }% on the creator revenue`}
      </p>
      <p>
        {`Refer a fan - get ${
          settings?.p2uReferralCommission * 100 || 0
        }% on the fan spends for 1 year`}
      </p>
    </div>
  );
  ...
  return (
    <Layout>
      <Head>
        <title>{ui && ui.siteName} | Referral</title>
      </Head>
...
export default connect(mapStates)(ModelReferralPage);
