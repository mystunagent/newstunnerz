...
class ProductUpdate extends PureComponent<IProps> {
  ...
  async componentDidMount() {
    ...
    if (
      ...
      !currentUser?.infoSubPerformer?.privilege.includes("products")
    ) {
      Router.push("/");
      return;
    }
  ...
  render() {
    const { product, submiting, fetching, uploadPercentage } = this.state;
    const { ui } = this.props;
    return (
      <Layout>
        <Head>
          <title>{ui && ui.siteName} | Edit Product</title>
        </Head>
