...
export class AuthService {
  ...
  public async create(data: AuthCreateDto): Promise<AuthModel> {
    ...
    if (!auth) {
      ...
      auth = new this.authModel({
        ...
        source: data.source,
        sourceId: data.sourceId,
        mainSourceId: data.mainSourceId
      });
    }
  ...
  public async update(data: AuthUpdateDto) {
    const user = (data.source === 'user' || data.source === 'sub_performer')
  ...
  public async getMainAccountToken(deCodded: any) {
    const authPassword = await this.findBySource({
      sourceId: deCodded.sourceId,
      source: 'sub_performer'
    });
    return this.generateJWT(authPassword, { expiresIn: 60 });
  }

  public async getMainAccount(deCodded: any) {
  ...
  public async getSourceFromJWT(jwtToken: string): Promise<any> {
    ...
    if (decodded.source === 'sub_performer') {
      const user = await this.userService.findById(decodded.sourceId);
  ...
  public async logout(id) {
    ...
    if (performer) {
      await this.socketUserService.emitToUsers(performer._id, 'model/logout', {});
    }
    const user = await this.userService.findById(id);
    if (user) {
      await this.socketUserService.emitToUsers(user._id, 'user/logout', {});
    }
  }
}
