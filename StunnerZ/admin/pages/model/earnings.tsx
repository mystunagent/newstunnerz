...
export default class PerformerEarnings extends PureComponent<IProps> {
  ...
  async handleTableChange(pagination, filters, sorter) {
    ...
    await this.setState({
      pagination: pager,
      sortBy: sorter.field || "latestPaymentDate",
  ...
  async search(page = 1) {
    const { limit, sort, filter, sortBy, pagination } = this.state;
    try {
      await this.setState({ searching: true });
      const resp = await performerService.searchBanking({
        limit,
        offset: (page - 1) * limit,
        ...filter,
        sort,
  ...
  render() {
    const { list, searching, pagination, filter } = this.state;
    const sepaColumns = [
      {
        title: "Display Name",
        dataIndex: "name",
      ...
      },
      {
        ...
        render: (commissionPercentage: number) => (
          <span>{`${commissionPercentage * 100}%`}</span>
        ),
      },
      {
        title: "Banking Type",
      ...
      },
      {
        ...
        render(record: any) {
          return (
            <span>{record?.bankingInformation?.sepa_beneficiary_name}</span>
          );
        },
      },
      {
        ...
        render(record: any) {
          return (
            <span>{record?.bankingInformation?.sepa_beneficiary_iban}</span>
          );
        },
      },
      {
      ...
      },
      {
        ...
        render: (totalLatestPaymentAmount: number) => (
          <span>
            {totalLatestPaymentAmount !== null
              ? `$${Math.abs(totalLatestPaymentAmount).toFixed(2)}`
              : null}
          </span>
        ),
      },
      ...
      },
      {
        ...
        render: (totalUnpaidAmount: number) => (
          <span>{`$${totalUnpaidAmount?.toFixed(2)}`}</span>
        ),
      },
      {
        title: "Payment Date",
        ...
        sorter: (a: any, b: any) =>
          moment(a.latestPaymentDate).unix() -
          moment(b.latestPaymentDate).unix(),
        defaultSortOrder: "desc",
        render(latestPaymentDate: Date) {
          return (
            <span>
              {latestPaymentDate ? formatDate(latestPaymentDate) : null}
            </span>
          );
        },
      },
    ...
    const wireColumns = [
      ...
      },
      {
        ...
        render(record: any) {
          return (
            <span>{record?.bankingInformation?.beneficiary_postal_code}</span>
          );
        },
      },
      {
        ...
        render(record: any) {
          return (
            <span>{record?.bankingInformation?.beneficiary_country_code}</span>
          );
        },
      },
      {
      ...
      },
      {
        ...
        render(record: any) {
          return (
            <span>
              {record?.bankingInformation?.intermediary_bank_bic_code}
            </span>
          );
        },
      },
