...
import {
  Form, Menu, message, Button, Input, InputNumber, Switch, Checkbox, Radio, Select
} from 'antd';
      ...
      case 'dropdown':
        return (
          <Form.Item label={setting.name} key={setting._id} help={setting.description} extra={setting.extra}>
            <Select onChange={(val) => this.setVal(setting.key, val)} defaultValue={setting.value}>
              {setting.options?.map((option) => option?.key && (
                <Select.Option value={option.key} key={option.key}>{option?.name}</Select.Option>
              ))}
            </Select>
          </Form.Item>
        );
        ...
        <Page>
          <div style={{ marginBottom: '20px' }}>
            <Menu onClick={this.onMenuChange.bind(this)} selectedKeys={[selectedTab]} mode="horizontal">
              ...
              <Menu.Item key="advertise">Advertise</Menu.Item>
              {/* <Menu.Item key="jumio">Jumio</Menu.Item> */}
              <Menu.Item key="ondato">Ondato</Menu.Item>
              <Menu.Item key="agora">Agora Live</Menu.Item>
              <Menu.Item key="paymentGateways">Verotel</Menu.Item>
